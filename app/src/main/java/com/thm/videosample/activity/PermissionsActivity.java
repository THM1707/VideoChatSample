...
public class PermissionsActivity extends AppCompatActivity {
    ...
    public static void startActivity(Activity activity, boolean checkOnlyAudio,
                                     String... permissions) {
    ...
    private void showDeniedResponse(int[] grantResults) {
        if (grantResults.length > 1) {
            for (int i = 0; i < grantResults.length; i++) {
                if (grantResults[i] != 0) {
                    Toast.makeText(this,
                        getString(R.string.message_permission_unavailable, permissionFeatures.values()[i]),
                        Toast.LENGTH_SHORT).show();
                }
            }
        } else {
            Toast.makeText(this,
                getString(R.string.message_permission_unavailable, permissionFeatures.MICROPHONE),
                Toast.LENGTH_SHORT).show();
        }
    }
